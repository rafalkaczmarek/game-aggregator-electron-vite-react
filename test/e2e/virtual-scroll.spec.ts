...
test.describe('virtual scroll', () => {
  ...
  test('grid view keeps a scrollable container and renders a subset of games', async ({ page }) => {
    ...
    await expect(page.getByText(new RegExp(`Last scan:.*— ${LARGE_LIBRARY_SIZE} games`))).toBeVisible()
  ...
  test('grid view reveals the last game after scrolling to the bottom', async ({ page }) => {
    ...
    await expect(grid.getByText('Game 0000')).toBeVisible()
    await expect(grid.getByText(`Game ${String(LARGE_LIBRARY_SIZE - 1).padStart(4, '0')}`)).toHaveCount(
      0,
    )
    ...
    await expect(grid.getByText(`Game ${String(LARGE_LIBRARY_SIZE - 1).padStart(4, '0')}`)).toBeVisible()
  ...
  test('list view reveals the last game after scrolling to the bottom', async ({ page }) => {
    ...
    const list = page.getByTestId('game-library-list')
    await expect(list.getByText('Game 0000')).toBeVisible()
    await expect(list.getByText(`Game ${String(LARGE_LIBRARY_SIZE - 1).padStart(4, '0')}`)).toHaveCount(
      0,
    )
    ...
    await expect(list.getByText(`Game ${String(LARGE_LIBRARY_SIZE - 1).padStart(4, '0')}`)).toBeVisible()
  ...
  test('platform filter still virtualizes the filtered library', async ({ page }) => {
    ...
    await expect(page.getByText(new RegExp(`Last scan:.*— ${LARGE_LIBRARY_SIZE} games \\(filtered\\)`))).toBeVisible()
